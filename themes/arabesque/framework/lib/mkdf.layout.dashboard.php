...
/*
   Class: ArabesqueMikadoDashboardForm
   A class that initializes Mikado Dashboard Container
*/

class ArabesqueMikadoDashboardForm implements iArabesqueMikadoLayoutNode, iArabesqueMikadoRender {
	public $children;
	public $name;
	public $form_id;
	public $form_method;
	...
	public function render( $factory ) {
        ...
        <form method="<?php echo esc_attr( $this->form_method ); ?>" id="<?php echo esc_attr( $this->form_id ); ?>" class="<?php echo esc_attr( $action_class ) ?>" data-action="<?php echo esc_attr( $this->form_action ); ?>">
			...
			if ( arabesque_mikado_core_plugin_installed() ) {
				echo arabesque_mikado_get_button_html( array(
...
/*
   Class: ArabesqueMikadoDashboardGroup
   A class that initializes ArabesqueMikado Group Field
*/

class ArabesqueMikadoDashboardGroup implements iArabesqueMikadoLayoutNode, iArabesqueMikadoRender {
	public $children;
	public $name;
	public $title;
	public $description;
...
/*
   Class: ArabesqueMikadoDashboardTitle
   A class that initializes Dashboard Title
*/

class ArabesqueMikadoDashboardTitle implements iArabesqueMikadoRender {
	private $name;
	private $title;
...
/*
   Class: ArabesqueMikadoDashboardField
   A class that initializes ArabesqueMikado Front Field
*/

class ArabesqueMikadoDashboardField implements iArabesqueMikadoRender {
	private $type;
	private $name;
	private $label;
	private $description;
...
abstract class ArabesqueMikadoDashboardFieldType {
	abstract public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = "" );
}

class ArabesqueMikadoDashboardFieldText extends ArabesqueMikadoDashboardFieldType {
...
class ArabesqueMikadoDashboardFieldTextArea extends ArabesqueMikadoDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
...
class ArabesqueMikadoDashboardFieldImage extends ArabesqueMikadoDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$class = '';
            ...
            <div class="mkdf-dashboard-gallery-holder">
                <ul class="mkdf-dashboard-gallery-images-holder">
					<?php if ( isset( $value_html ) ) {
						echo arabesque_mikado_get_module_part( $value_html );
					} ?>
                </ul>
                <div class="mkdf-dashboard-gallery-uploader">
					...
					if ( mkdf_membership_theme_installed() ) {
						echo arabesque_mikado_get_button_html( array(
							'text'         => esc_html__( 'Upload', 'arabesque' ),
							'custom_class' => 'mkdf-dashboard-gallery-upload'
						) );
					} else {
						echo '<a itemprop="url" href="#" target="_self" class="mkdf-btn mkdf-btn-medium mkdf-btn-solid mkdf-dashboard-gallery-upload"><span class="mkdf-btn-text">' . esc_html__( 'Upload', 'arabesque' ) . '</span></a>';
					} ?>
					...
					<?php if ( $value !== '' && $value !== false ) { ?>
                        <button class="mkdf-btn mkdf-btn-solid mkdf-dashboard-remove-image"><?php esc_html_e( 'Remove Media', 'arabesque' ); ?></button>
					<?php } ?>
                </div>
            </div>
        </div>
...
class ArabesqueMikadoDashboardFieldGallery extends ArabesqueMikadoDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$class = '';
...
class ArabesqueMikadoDashboardFieldSelect extends ArabesqueMikadoDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$class = '';
...
class ArabesqueMikadoDashboardFieldDate extends ArabesqueMikadoFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		if ( ! empty( $repeat ) && array_key_exists( 'name', $repeat ) && array_key_exists( 'index', $repeat ) ) {
			$id   = $name . '-' . $repeat['index'];
			$name = $repeat['name'] . '[' . $repeat['index'] . '][' . $name . ']';
...
class ArabesqueMikadoDashboardFieldIcon extends ArabesqueMikadoDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$options           = arabesque_mikado_icon_collections()->getIconCollectionsEmpty();
...
class ArabesqueMikadoDashboardFieldColor extends ArabesqueMikadoDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		if ( ! empty( $repeat ) && array_key_exists( 'name', $repeat ) && array_key_exists( 'index', $repeat ) ) {
			$id   = $name . '-' . $repeat['index'];
			$name = $repeat['name'] . '[' . $repeat['index'] . '][' . $name . ']';
...
class ArabesqueMikadoDashboardFieldCheckBoxGroup extends ArabesqueMikadoDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
...
class ArabesqueMikadoDashboardFieldAddress extends ArabesqueMikadoFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
		if ( isset( $args["col_width"] ) ) {
			$col_width = $args["col_width"];
...
class ArabesqueMikadoDashboardRepeater implements iArabesqueMikadoRender {
	private $label;
	private $description;
	private $name;
	private $fields;
			...
			<?php if ( $this->table_layout ) { ?>
                <div class="mkdf-dashboard-repeater-table-heading">
                    <div class="mkdf-dashboard-repeater-fields-holder">
                        <div class="mkdf-dashboard-repeater-table-cell mkdf-dashboard-repeater-sort"><?php esc_html_e( 'Order', 'arabesque' ) ?></div>
                        ...
                        <div class="mkdf-dashboard-repeater-table-cell mkdf-dashboard-repeater-remove"><?php esc_html_e( 'Remove', 'arabesque' ) ?></div>
                    </div>
                </div>
			<?php } ?>
...
class ArabesqueMikadoDashboardFieldFactory {
	public function render( $field_type, $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		switch ( strtolower( $field_type ) ) {
			case 'text':
				$field = new ArabesqueMikadoDashboardFieldText();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'textarea':
				$field = new ArabesqueMikadoDashboardFieldTextArea();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'date':
				$field = new ArabesqueMikadoDashboardFieldDate();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'image':
				$field = new ArabesqueMikadoDashboardFieldImage();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'gallery':
				$field = new ArabesqueMikadoDashboardFieldGallery();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'select':
				$field = new ArabesqueMikadoDashboardFieldSelect();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'icon':
				$field = new ArabesqueMikadoDashboardFieldIcon();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'color':
				$field = new ArabesqueMikadoDashboardFieldColor();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'checkboxgroup':
				$field = new ArabesqueMikadoDashboardFieldCheckBoxGroup();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'address':
				$field = new ArabesqueMikadoDashboardFieldAddress();
