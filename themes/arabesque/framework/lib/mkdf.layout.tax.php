...
/*
   Class: ArabesqueMikadoTaxonomyField
   A class that initializes ArabesqueMikado Taxonomy Field
*/

class ArabesqueMikadoTaxonomyField implements iArabesqueMikadoRender {
	private $type;
	private $name;
	private $label;
	private $description;
	...
	function __construct( $type, $name, $label = "", $description = "", $options = array(), $args = array(), $hidden_property = "", $hidden_values = array() ) {
		...
		add_filter( 'arabesque_mikado_filter_taxonomy_fields', array( $this, 'addFieldForEditSave' ) );
	}
...
abstract class ArabesqueMikadoTaxonomyFieldType {
	abstract public function render( $name, $label = "", $description = "", $options = array(), $args = array() );
}

class ArabesqueMikadoTaxonomyFieldText extends ArabesqueMikadoTaxonomyFieldType {
...
class ArabesqueMikadoTaxonomyFieldImage extends ArabesqueMikadoTaxonomyFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $hidden = false) {
		if ( ! isset( $_GET['tag_ID'] ) ) { ?>
			<div class="form-field">
				<label for="<?php echo esc_attr( $name ); ?>"><?php echo esc_html( $label ); ?></label>
				...
				<p>
					<input type="button" class="button button-secondary mkdf-tax-media-add" name="mkdf-tax-media-add" value="<?php esc_attr_e( 'Add Image', 'arabesque' ); ?>"/>
					<input type="button" class="button button-secondary mkdf-tax-media-remove" name="mkdf-tax-media-remove" value="<?php esc_attr_e( 'Remove Image', 'arabesque' ); ?>"/>
				</p>
			</div>
			<?php
		} else {
			...
			<tr class="form-field" <?php if ($hidden) { ?> style="display: none"<?php } ?>>
				...
				<td>
					...
					<p>
						<input type="button" class="button button-secondary mkdf-tax-media-add" name="mkdf-tax-media-add" value="<?php esc_attr_e( 'Add Image', 'arabesque' ); ?>"/>
						<input data-termid="<?php echo esc_attr( $_GET['tag_ID'] ); ?>" data-taxonomy="<?php echo esc_attr( $taxonomy ); ?>" type="button" class="button button-secondary mkdf-tax-media-remove" name="mkdf-tax-media-remove" value="<?php esc_attr_e( 'Remove Image', 'arabesque' ); ?>"/>
					</p>
				</td>
			</tr>
			<?php
...
class ArabesqueMikadoTaxonomyFieldSelect extends ArabesqueMikadoTaxonomyFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $hidden = false ) {
...
class ArabesqueMikadoTaxonomyFieldIcon extends ArabesqueMikadoTaxonomyFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $hidden = false ) {
		...
		if ( ! isset( $_GET['tag_ID'] ) ) { ?>
			...
			<?php foreach ( $icons_collections as $icons_collection ) {
				$icons_param = arabesque_mikado_icon_collections()->getIconCollectionParamNameByKey( $icons_collection );
				?>
                <div class="form-field mkdf-icon-collection-holder" style="display: none" data-icon-collection="<?php echo esc_attr( $icons_collection ); ?>">
                    <label for="<?php echo esc_attr( $name ) . '_icon'; ?>"><?php esc_html_e( 'Icon', 'arabesque' ); ?></label>
			...
			<?php foreach ( $icons_collections as $icons_collection ) {
				...
				?>
                <tr class="form-field mkdf-icon-collection-holder" style="<?php echo esc_attr( $style ); ?>" data-icon-collection="<?php echo esc_attr( $icons_collection ); ?>">
                    <th scope="row"><?php esc_html_e( 'Icon', 'arabesque' ); ?></th>
                    <td>
                        <select name="<?php echo esc_attr( $name . '_' . $icons_param ) ?>" id="<?php echo esc_attr( $name . '_' . $icons_param ) ?>">
							<?php
							$icons      = arabesque_mikado_icon_collections()->getIconCollection( $icons_collection );
...
class ArabesqueMikadoTaxonomyFieldColor extends ArabesqueMikadoTaxonomyFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $hidden = false ) {
...
class ArabesqueMikadoTaxonomyFieldFactory {
	public function render( $field_type, $name, $label = "", $description = "", $options = array(), $args = array(), $hidden = false ) {
		
		switch ( strtolower( $field_type ) ) {
			case 'text':
				$field = new ArabesqueMikadoTaxonomyFieldText();
				$field->render( $name, $label, $description, $options, $args, $hidden );
				break;
			
			case 'image':
				$field = new ArabesqueMikadoTaxonomyFieldImage();
				$field->render( $name, $label, $description, $options, $args, $hidden );
				break;
			
			case 'selectblank':
				$field = new ArabesqueMikadoTaxonomyFieldSelect();
				$field->render( $name, $label, $description, $options, $args, $hidden );
				break;
			
			case 'icon':
				$field = new ArabesqueMikadoTaxonomyFieldIcon();
				$field->render( $name, $label, $description, $options, $args, $hidden );
				break;
			
			case 'color':
				$field = new ArabesqueMikadoTaxonomyFieldColor();
				$field->render( $name, $label, $description, $options, $args, $hidden );
				break;
