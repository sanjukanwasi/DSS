...
class MikadoSkin extends ArabesqueMikadoSkinAbstract {
	
	/**
	 * Skin constructor. Hooks to arabesque_mikado_admin_scripts_init and arabesque_mikado_enqueue_admin_styles
	 */
	public function __construct() {
		$this->skinName = 'mikado';
		
		// Admin register scripts and styles
		add_action( 'arabesque_mikado_action_admin_scripts_init', array( $this, 'registerStyles' ) );
		add_action( 'arabesque_mikado_action_admin_scripts_init', array( $this, 'registerScripts' ) );
		
		// Admin enqueue scripts and styles
		add_action( 'arabesque_mikado_action_enqueue_admin_styles', array( $this, 'enqueueStyles' ) );
		add_action( 'arabesque_mikado_action_enqueue_admin_scripts', array( $this, 'enqueueScripts' ) );
		
		// Admin enqueue meta box scripts and styles
		add_action( 'arabesque_mikado_action_enqueue_meta_box_styles', array( $this, 'enqueueStyles' ) );
		add_action( 'arabesque_mikado_action_enqueue_meta_box_scripts', array( $this, 'enqueueScripts' ) );
		
		// Change options map order for this skin
		add_filter( 'arabesque_mikado_filter_options_map_position', array( $this, 'setOptionsMapPosition' ), 10, 2 );
	...
	public function registerScripts() {
		...
		foreach ( $this->scripts as $scriptHandle => $script ) {
			arabesque_mikado_register_skin_script( $scriptHandle, $script['path'], $script['dependency'] );
		}
	}
    ...
    public function registerStyles() {
        ...
        foreach ($this->styles as $styleHandle => $stylePath) {
            arabesque_mikado_register_skin_style($styleHandle, $stylePath);
        }
    }
    ...
    public function renderOptions() {
        ...
        $tab    		= arabesque_mikado_get_admin_tab();
        $active_page 	= $arabesque_mikado_Framework->mkdfOptions->getAdminPageFromSlug($tab);
     ...
     *
     * @see ArabesqueMikadoSkinAbstract::loadTemplatePart()
     */
     ...
     *
     * @param ArabesqueMikadoClassAdminPage $page current page to load
	...
	public function renderBackupOptions() { ?>
		<div class="mkdf-options-page mkdf-page">
			<?php $this->getHeader(arabesque_mikado_get_theme_info_item('Name'), arabesque_mikado_get_theme_info_item('Version'), false); ?>
			<div class="mkdf-page-content-wrapper">
				<div class="mkdf-page-content">
					<div class="mkdf-page-navigation mkdf-tabs-wrapper vertical left clearfix">
