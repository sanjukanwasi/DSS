...
if ( class_exists( 'ArabesqueMikadoWidget' ) ) {
	class ArabesqueMikadoSideAreaOpener extends ArabesqueMikadoWidget {
		public function __construct() {
			parent::__construct(
				'mkdf_side_area_opener',
				esc_html__(
					'Arabesque Side Area Opener',
					'arabesque'
				),
				array(
		...
		public function widget( $args, $instance ) {
			...
			<a <?php arabesque_mikado_class_attribute( $classes ); ?> <?php echo arabesque_mikado_get_inline_attr(
				$instance['icon_hover_color'],
				'data-hover-color'
			); ?> href="javascript:void(0)" <?php arabesque_mikado_inline_style( $styles ); ?>>
				<?php if ( ! empty( $instance['widget_title'] ) ) { ?>
					<h5 class="mkdf-side-menu-title"><?php echo esc_html( $instance['widget_title'] ); ?></h5>
				<?php } ?>
				<span class="mkdf-side-menu-icon">
				<?php echo arabesque_mikado_get_icon_sources_html( 'side_area' ); ?>
            </span>
			</a>
		<?php }
	}
