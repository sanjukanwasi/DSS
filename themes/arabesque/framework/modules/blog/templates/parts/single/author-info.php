<?php
$author_info_box       = esc_attr( arabesque_mikado_options()->getOptionValue( 'blog_author_info' ) );
$author_info_email     = esc_attr( arabesque_mikado_options()->getOptionValue( 'blog_author_info_email' ) );
$author_id             = esc_attr( get_the_author_meta( 'ID' ) );
$social_networks       = arabesque_mikado_core_plugin_installed() ? arabesque_mikado_get_user_custom_fields() : false;
$display_author_social = arabesque_mikado_options()->getOptionValue( 'blog_single_author_social' ) === 'no' ? false : true;
?>
			...
			<div class="mkdf-author-description-content clearfix">
				<div class="mkdf-author-description-image">
					<a itemprop="url" href="<?php echo esc_url( get_author_posts_url( $author_id ) ); ?>" title="<?php the_title_attribute(); ?>" target="_self">
						<?php echo arabesque_mikado_kses_img( get_avatar( get_the_author_meta( 'ID' ), 162 ) ); ?>
					</a>
				</div>
							...
							<div class="mkdf-author-social-icons clearfix">
								<?php foreach ( $social_networks as $network ) { ?>
									<a itemprop="url" href="<?php echo esc_url( $network['link'] ) ?>" target="_blank">
										<?php echo arabesque_mikado_icon_collections()->renderIcon( $network['class'], 'font_elegant' ); ?>
									</a>
								<?php } ?>
							</div>
						<?php } ?>
