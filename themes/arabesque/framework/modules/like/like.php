...
class ArabesqueMikadoLike {
	private static $instance;

	private function __construct() {
		add_action( 'wp_ajax_arabesque_mikado_like', array( $this, 'ajax' ) );
		add_action( 'wp_ajax_nopriv_arabesque_mikado_like', array( $this, 'ajax' ) );
	}
	...
	public function add_like() {
		...
		$title       = esc_html__( 'Like this', 'arabesque' );

		if ( isset( $_COOKIE[ 'mkdf-like_' . $post->ID ] ) ) {
			$class = 'mkdf-like liked';
			$title = esc_html__( 'You already like this!', 'arabesque' );
		}
...
if ( ! function_exists( 'arabesque_mikado_create_like' ) ) {
	function arabesque_mikado_create_like() {
		ArabesqueMikadoLike::get_instance();
	}

	add_action( 'after_setup_theme', 'arabesque_mikado_create_like' );
}