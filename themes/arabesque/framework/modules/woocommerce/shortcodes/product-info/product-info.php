...
namespace ArabesqueCore\CPT\Shortcodes\ProductInfo;
...
class ProductInfo implements Lib\ShortcodeInterface {
	...
	function __construct() {
		...
		//Product id filter
		add_filter( 'vc_autocomplete_mkdf_product_info_product_id_callback', array(
			&$this,
			'productIdAutocompleteSuggester',
		), 10, 1 );

		//Product id render
		add_filter( 'vc_autocomplete_mkdf_product_info_product_id_render', array(
			&$this,
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Product Info', 'arabesque' ),
					'base'                      => $this->getBase(),
					...
					'params'                    => array(
						array(
							...
							'heading'     => esc_html__( 'Selected Product', 'arabesque' ),
							'settings'    => array(
								'sortable'      => true,
								'unique_values' => true
							),
							'admin_label' => true,
							'save_always' => true,
							'description' => esc_html__( 'If you left this field empty then product ID will be of the current page', 'arabesque' )
						),
						array(
							'type'        => 'dropdown',
							'param_name'  => 'product_info_type',
							'heading'     => esc_html__( 'Product Info Type', 'arabesque' ),
							'value'       => array(
								esc_html__( 'Title', 'arabesque' )          => 'title',
								esc_html__( 'Featured Image', 'arabesque' ) => 'featured_image',
								esc_html__( 'Category', 'arabesque' )       => 'category',
								esc_html__( 'Excerpt', 'arabesque' )        => 'excerpt',
								esc_html__( 'Price', 'arabesque' )          => 'price',
								esc_html__( 'Rating', 'arabesque' )         => 'rating',
								esc_html__( 'Add To Cart', 'arabesque' )    => 'add_to_cart',
								esc_html__( 'Tag', 'arabesque' )            => 'tag',
								esc_html__( 'Author', 'arabesque' )         => 'author',
						...
						array(
							...
							'heading'    => esc_html__( 'Product Info Color', 'arabesque' ),
							'dependency' => array(
								'element' => 'product_info_type',
								'value'   => array(
									'title',
									'category',
									'excerpt',
						...
						array(
							...
							'value'       => array_flip( arabesque_mikado_get_title_tag( true, array( 'p' => 'p' ) ) ),
							'description' => esc_html__( 'Set title tag for product title element', 'arabesque' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Featured Image Proportions', 'arabesque' ),
							'value'      => array(
							...
							'dependency' => array(
								'element' => 'product_info_type',
								'value'   => array( 'featured_image' )
							)
						),
						array(
							'type'        => 'dropdown',
							'param_name'  => 'category_tag',
							'heading'     => esc_html__( 'Category Tag', 'arabesque' ),
							'value'       => array_flip( arabesque_mikado_get_title_tag( true ) ),
							'description' => esc_html__( 'Set category tag for product category element', 'arabesque' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Add To Cart Skin', 'arabesque' ),
							'value'      => array(
								esc_html__( 'Default', 'arabesque' ) => '',
								esc_html__( 'White', 'arabesque' )   => 'white',
						...
						array(
							...
							'heading'    => esc_html__( 'Add To Cart Size', 'arabesque' ),
							'value'      => array(
								esc_html__( 'Default', 'arabesque' ) => '',
							...
							'dependency' => array( 'element' => 'product_info_type', 'value' => array( 'arabesque' ) ),
							'group'      => esc_html__( 'Product Info Style', 'arabesque' )
						)
					)
				)
	...
	public function getItemTitleHtml( $params ) {
		...
		if ( ! empty( $title ) ) {
			$html = '<' . esc_attr( $title_tag ) . ' itemprop="name" class="mkdf-pi-title entry-title" ' . arabesque_mikado_get_inline_style( $product_info_style ) . '>';
		...
		if ( ! empty( $categories ) ) {
			...
			foreach ( $categories as $cat ) {
				if ( ! empty( $params['category_tag'] ) ) {
					$html .= '<' . esc_attr( $params['category_tag'] ) . ' ' . arabesque_mikado_get_inline_style( $product_info_style ) . '>';
					...
					$html .= '<a itemprop="url" class="mkdf-pi-category-item" href="' . esc_url( get_term_link( $cat->term_id ) ) . '" ' . arabesque_mikado_get_inline_style( $product_info_style ) . '>' . esc_html( $cat->name ) . '</a>';
				}
			}
			$html .= '</div>';
		}
	...
	public function getItemExcerptHtml( $params ) {
		...
		if ( ! empty( $excerpt ) ) {
			$html = '<div class="mkdf-pi-excerpt"><p itemprop="description" class="mkdf-pi-excerpt-item" ' . arabesque_mikado_get_inline_style( $product_info_style ) . '>' . esc_html( $excerpt ) . '</p></div>';
		}
	...
	public function getItemPriceHtml( $params ) {
		...
		if ( $price_html = $product->get_price_html() ) {
			$html = '<div class="mkdf-pi-price" ' . arabesque_mikado_get_inline_style( $product_info_style ) . '>' . $price_html . '</div>';
		}
	...
	public function getItemRatingHtml( $params ) {
		...
		if ( get_option( 'woocommerce_enable_review_rating' ) !== 'no' ) {
			...
			$html = '<div class="mkdf-pi-rating" title="' . sprintf( esc_attr__( "Rated %s out of 5", "arabesque" ), $average ) . '" ' . arabesque_mikado_get_inline_style( $product_info_style ) . '><span style="width:' . ( ( $average / 5 ) * 100 ) . '%"></span></div>';
		}
	...
	public function getItemAddToCartHtml( $params ) {
		...
		$html .= apply_filters( 'arabesque_mikado_filter_product_info_add_to_cart_link',
	...
	public function getItemTagHtml( $params ) {
		...
		if ( ! empty( $tags ) ) {
			...
			foreach ( $tags as $tag ) {
				$html .= '<a itemprop="url" class="mkdf-pi-tag-item" href="' . esc_url( get_term_link( $tag->term_id ) ) . '" ' . arabesque_mikado_get_inline_style( $product_info_style ) . '>' . esc_html( $tag->name ) . '</a>';
			}
			$html .= '</div>';
		}
	...
	public function getItemAuthorHtml( $params ) {
		...
		if ( ! empty( $author ) ) {
			$html = '<div class="mkdf-pi-author" ' . arabesque_mikado_get_inline_style( $product_info_style ) . '>' . esc_html( $author ) . '</div>';
		}
	...
	public function getItemDateHtml( $params ) {
		...
		if ( ! empty( $date ) ) {
			$html = '<div class="mkdf-pi-date" ' . arabesque_mikado_get_inline_style( $product_info_style ) . '>' . esc_html( $date ) . '</div>';
		}
	...
	public function productIdAutocompleteSuggester( $query ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				$data          = array();
				$data['value'] = $value['id'];
				$data['label'] = esc_html__( 'Id', 'arabesque' ) . ': ' . $value['id'] . ( ( strlen( $value['title'] ) > 0 ) ? ' - ' . esc_html__( 'Title', 'arabesque' ) . ': ' . $value['title'] : '' );
				$results[]     = $data;
			}
		}
		...
		if ( ! empty( $query ) ) {
			...
			if ( ! is_wp_error( $product ) ) {
				...
				if ( ! empty( $product_title ) ) {
					$product_title_display = ' - ' . esc_html__( 'Title', 'arabesque' ) . ': ' . $product_title;
				}

				$product_id_display = esc_html__( 'Id', 'arabesque' ) . ': ' . $product_id;
