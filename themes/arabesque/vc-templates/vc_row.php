...
$row_content_width = $anchor = $content_text_aligment = $simple_background_color = $simple_background_image = $background_image_position = $disable_background_image = $parallax_background_image = $parallax_bg_speed = $parallax_bg_height = $row_btt_skin = '';
...
if ( $row_content_width !== 'grid' ) {
    ...
    if (!empty($row_btt_skin)){
        $css_classes[] = ($row_btt_skin) == 'light' ? 'mkdf-row-btt-light' : '';
    }
    ...
    if (!empty($row_btt_skin)){
        $grid_row_class[] = ($row_btt_skin) == 'light' ? 'mkdf-row-btt-light' : '';
    }
}
...
if ( $row_content_width === 'grid' ) {
	$mkdf_row_wrapper_start .= '<div class="mkdf-row-grid-section-wrapper ' . esc_attr( $grid_row_classes ) . '" ' . esc_attr( $grid_row_datas ) . ' ' . arabesque_mikado_get_inline_style( $mkdf_grid_row_styles ) . '><div class="mkdf-row-grid-section">';
...
$output .= '<div ' . implode( ' ', $wrapper_attributes ) . ' ' . arabesque_mikado_get_inline_style( $mkdf_vc_row_styles ) . '>';
$output .= wpb_js_remove_wpautop( $content );
$output .= '</div>';
$output .= $mkdf_row_wrapper_end;
$output .= $after_output;

echo arabesque_mikado_get_module_part( $output );
